...
(function () {
  ...
  const { mulberry32, shuffle } = NS.RNG;

  const SUIT = { CLUB: "♣", SPADE: "♠", DIAMOND: "♦", HEART: "♥" };
  const TYPE = { MONSTER: "monster", WEAPON: "weapon", POTION: "potion" };
  ...
  function buildDeck() {
    const cards = [];
    for (let v = 2; v <= 14; v++) {
      cards.push(make(`m-c-${v}`, TYPE.MONSTER, SUIT.CLUB,  v));
      cards.push(make(`m-s-${v}`, TYPE.MONSTER, SUIT.SPADE, v));
    }
    for (let v = 2; v <= 10; v++) {
  ...
  NS.Deck = {
    SUIT, TYPE,
    rankLabel, cardLabel,
    imageForCard, artworkName,
    buildDeck, buildDungeon,
  };
})();
